...
c Find the object on the master list
            unit(k) = 0
            do l = 1, nmaster  !Bottleneck here
...
c If output is required at this epoch, write elements to appropriate files
          if (firstflag.eq.0.or.abs(time-tprevious).ge.teval) then  !Bottleneck: wait, wait, wait, so the orbital elements are calculated at *each* available time step, not only at the time steps we actually print out?
            firstflag = 1
            tprevious = time
c
c Write required elements to the appropriate aei file
            do j = 1, nbod
              k = code(j)
              if (unit(k).ge.10) then !So only the open units get written to
...
c If some objects remain on waiting list, read through input files again
      if (nwait.gt.0) then !Bottleneck: goes back, loops over each time slice again
        do j = 1, nmaster
...
! 213  format (1x,a8,1x,f8.4,1x,f7.5,1x,f7.3,1p,e11.4,0p,1x,f6.3,1x,f6.2)
 213  format (1x,a8,1x,f10.6,1x,f10.7,1x,f10.5,1p,e13.6,0p,1x,f6.3,
     % 1x,f6.2)
c
      end
c
