...
!
    ...
    integer(I4)::whichones
    real(R8)::temp_dotproduct
    logical::already_been_in_a_collision,other_already_been_in_a_collision
!------------------------------------------------------------------------------
...
! Identify pairs whose x-y boxes overlap, and calculate minimum separation
    closeencountersearch_base: do i = 1, nbig 
      already_been_in_a_collision = .false.
      do j = i + 1, n
        if (xmax(i) < xmin(j).or.xmax(j) < xmin(i).or. &
            ymax(i) < ymin(j).or.ymax(j) < ymin(i)) cycle
!
...
! Make linear interpolation to estimate coordinates at time of closest approach
            ...
            if (d2min <= d2hit) then
               ...
               other_already_been_in_a_collision = .false.
               do lll = 1, nhit
                   if ( (hit(lll) % i == itarg).or.(hit(lll) % j == itarg ).or. &
                        ( hit(lll) % i == iproj ).or. (hit(lll) % j == iproj) ) then
                      other_already_been_in_a_collision = .true.
                      exit !Finish the do statement
                   end if
               end do
               if ( (already_been_in_a_collision.eqv..false.).and.(other_already_been_in_a_collision.eqv..false.) ) then
                  nhit = nhit  +  1
                  hit(nhit) % i = itarg
                  hit(nhit) % j = iproj
                  hit(nhit) % t = tmin  +  t
                  ...
                  already_been_in_a_collision = .true.
               else
                  if ( (already_been_in_a_collision.eqv..true.).and.(other_already_been_in_a_collision.eqv..true.) ) then
                     whichones = 3
                  else if (already_been_in_a_collision.eqv..true.) then
                     whichones = 1
                  else
                     whichones = 2
                  end if
