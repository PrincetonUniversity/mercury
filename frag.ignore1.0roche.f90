...
! Maximum number of particles (can be changed by the user)
      integer(I4), parameter::NMAX = 4096
! Encounter data type:
...
!
! Time to remove interior particles
      real(R8), parameter::time_to_remove = 1.5e4_R8*YEAR
      logical::flag_notremoved
    end module globals
!==============================================================================
    module swift
      use kinds
...
!      status(iproj)   = 'big  '  ! The data are already there
       ...
       write (23,'(3a,es11.4)') '   Remnant:  ',name(iproj), '  m=', m(iproj) / MSUN
    endif
...
! Angular momentum
    ...
    h2 = dot_product(h, h)
!
! Relative velocity at impact, using conservation of energy
...
!    #endif
    ...
    character(5), intent(inout)::status(:)
!
    integer(I4)::i,j,k,ll,nclo,nhit,ncrit,ncrit_big,ncrit_old
...
! for eliminating bodies
    real(R8)::r,v2,gmsum,smaxis,energy1,angmom1,energy2,angmom2
!------------------------------------------------------------------------------
...
!
! Remove all particles interior to a certain point at a certain time
      if ((time >= time_to_remove).and.(flag_notremoved)) then
         write(*,*) "Removing bodies inside 1.0 Roche radii"
! Calculate initial energy
         call calc_energy (n,nbig,m,x,v,s,energy1,angmom1)
!
        do ll = 1, n
           r  = sqrt(dot_product(x(:,ll), x(:,ll)))
           v2 =      dot_product(v(:,ll), v(:,ll))
           gmsum = G * (mcen  +  m(ll))
!
           smaxis = gmsum * r / (2.0_R8 * gmsum  -  r * v2)
           if (smaxis > 0.0088781786_R8 * AU) status(ll) = 'dead '
        end do
!
        call remove_dead_bodies( n,nbig,m,x,v,s,ngf,rho,rce_hill,rad,status,index,name)
        ...
        rcrit(1:n) = calc_rcrit  (dt,n,m,x,v)
        flag_accel = .true.
        flag_notremoved = .false.
        write(*,*) "Number of big bodies at time ", time/YEAR, " years is: ", nbig
        call how_many_bodies_inside_andoutside_roche_radius(n,nbig,time/YEAR,x)
      end if
!
