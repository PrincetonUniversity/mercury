...
c        write (*,'(A6,E14.8)')'time: ',tstart/365.25
        write (*,'(E14.8)')rcrit(4)
        write (*,'(E14.8)')rcrit(5)
        write (*,'(E14.8)')rcrit(20)
        write (*,'(E14.8)')rcrit(51)
        write (*,'(E14.8)')rcrit(107)
      end if
c
...
c      write(*,'(A12,F6.1,A17,I5)')'time years: ', time/365.25, '  number
c     % of bodies: ', nbod
...
c If objects had close encounters, advance H_K using Bulirsch-Stoer instead
      if (nce.gt.0) then
c      write(*,'(A12,F6.1,A29,I5,A22,E12.4)')'time years: ', time/365.25, 
c     %'  number of close encounters: ', nce, ', random rcrit value: ', 
c     % rcrit(5)
        do j = 2, nbod
          if (ce(j).ne.0) then
            x(1,j) = x0(1,j)
